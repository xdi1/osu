...
{
    public class ManiaArgonMaxSkinTransformer : SkinTransformer
    {
        private readonly ManiaBeatmap beatmap;

        public ManiaArgonMaxSkinTransformer(ISkin skin, IBeatmap beatmap)
            : base(skin)
        {
            ...
            if (lookup is ManiaSkinConfigurationLookup maniaLookup)
            {
                ...
                switch (maniaLookup.Lookup)
                {
                    ...
                    case LegacyManiaSkinConfigurationLookups.ColumnWidth:
                        if (columnsCount >= 8) 
                        {
                            if (columnIndex == 0 || columnIndex == 1 || (columnIndex == columnsCount - 1) || (columnIndex == columnsCount - 2))
                            {
