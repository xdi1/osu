        ...
        private void onSkinChanged()
        {
            if (columnCount >= 8) {
                float? width = currentSkin.GetConfig<ManiaSkinConfigurationLookup, float>(
                                              new ManiaSkinConfigurationLookup(LegacyManiaSkinConfigurationLookups.ColumnWidth, 0))
                                          ?.Value;
                columnsST[0].Width = columnsST[1].Width = columnsFX[0].Width = columnsFX[1].Width = width.Value;
                columnsST[0].Margin = new MarginPadding { Left = 0 };
                columnsFX[0].Margin = new MarginPadding { Left = 0 };
                columnsST[1].Margin = new MarginPadding { Left = 2 };
                columnsFX[1].Margin = new MarginPadding { Left = 2 };
                for (int i = 0; i < columnCount - 4; i++)
                {
                    float spacing = 2;
                    width = currentSkin.GetConfig<ManiaSkinConfigurationLookup, float>(
                                              new ManiaSkinConfigurationLookup(LegacyManiaSkinConfigurationLookups.ColumnWidth, i+2))
                                          ?.Value;
                    if (i == 0) {
                        columns[i].Margin = new MarginPadding { Left = 0 };
                    }
                    else
                    {
